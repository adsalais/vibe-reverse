...
- **Python tools** (angr, z3) install into a **venv** at `$RE_HARNESS_VENV` on a
  host (the generated `install.sh` targets it), or **globally** in the container.
  For a one-shot full install of everything, see `requirements/setup.sh` (host) or
  `requirements/Dockerfile` (container).
